...
{
  std::cerr << "\033[1;34mFlag\033[0m:" << std::setprecision(4) << d << std::endl;
}
  ...
  for (int i=0; i < K; i++)
    {
      ...
      readIntxnFile(fname, spList[i], idList, id2sp, dummy);
    }
  dummy.clear();
}
...
typedef Str2Str::const_iterator It;
  ...
  int numVals = 0;
  std::cerr << "blast file: (for reading protein names only)" 
	    << fname << std::endl;
  std::ifstream ifs(fname.c_str());
				    ...
				    StrPair2Dbl & idpair2IntxnScore)
{
  ...
  */
  ...
  for (int i=0; i < K; i++)
      ...
      for (int j=i; j < K; j++)
	{
	  ...
	  readBlastScoresFile(fname, 
			      ...
			      idList, id2sp, 
			      idpair2BlastScore);
		printf("Finished : %d | Total : %d",times,count);
		fflush( stdout ) ;
		backspace( 63 ) ;
	times=times+1;
	}
  ...
  std::cerr << "The number of Blast Score totally: " << idpair2BlastScore.size()<< std::endl;
